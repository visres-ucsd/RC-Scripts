...
for (i in 1:length(XMLS)) {
  ...
  if (TYPE == "Visual Field 60-4 Test Pattern") {
    ...
    ff1   <- jFetch(c("Fixation Monitoring Code Sequence", "Code Meaning"), skippr = 2)
    ff2   <- jFetch(c("Fixation Monitoring Code Sequence", "Code Meaning"))
  ...
  if (TYPE == "Visual Field 24-2 Test Pattern") {
    ...
    linr <- c(
      ...
      jFetch(c("Performed Protocol Code Sequence", "Protocol Context Sequence", 
               "Concept Name Code Sequence", "Code Meaning")),
      rep("", 2),
      gsub(" Test Pattern", "", gsub("Visual Field ", "", TYPE, fixed = TRUE), fixed = TRUE), #46
      gsub(" Test Strategy", "", gsub("Visual Field ", "",
                                      jFetch(c("Performed Protocol Code Sequence", "Code Meaning"), 
                                             skippr = 3), fixed = TRUE), fixed = TRUE),
      jFetch(c("Stimulus Color Code Sequence", "Code Meaning")),
...
if (length(l242) > 0) {
  ...
  for (j in cNames242[!cNames242 %in% names(o242_FMP)]) {
    o242_FMP[, j] <- ""
  }
  ...
  # QC Files
  qc_cols <- c('Batch_UID', 'Exam_UID', 'InstrumentModel', 'InstrumentSerialNumber',                 #
  ...
  cols_242    <- tolower(gsub("_", "", names(o242), fixed = TRUE))
  cols_242_QC <- tolower(gsub("_", "", qc_cols, fixed = TRUE))

  idx <- which(cols_242 %in% cols_242_QC)
  
  o242_QC <- o242[, idx]
  colnames(o242_QC) <- cols_242[idx]
  
  o242_QC[, cols_242_QC[which(!cols_242_QC %in% names(o242_QC))]] <- ""

  o242_QC <- o242_QC[, cols_242_QC]
  colnames(o242_QC) <- qc_cols
