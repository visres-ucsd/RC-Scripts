...
####################################################################################################
####################################################################################################
## Parsing Forum XML's VERSION 1.19                                                               ##
####################################################################################################
####################################################################################################
####################################################################################################
####################################################################################################
...
## Version 1.0: Support for 10-2, 24-2, 30-2 and 60-4
...
for (i in 1:length(XMLS)) {
  ...
  sens <- jFetch("Sensitivity Value", all = TRUE)
  ...
  if (TYPE == "Visual Field 30-2 Test Pattern") {
    ...
    ## Series Values
    tds  <- jFetch("Age Corrected Sensitivity Deviation Value", all = TRUE)
    pds  <- jFetch("Generalized Defect Corrected Sensitivity Deviation Value", all = TRUE)
...
if (length(l242) > 0) {
  ...
  reg_cols <- c('hfa2softwareversion', 'birthdate', 'visitdate', 'site', 'instrumentmodelnumber', 
                'examtime', 'trialrxsphere', 'trialrxcylinder', 'trialrxaxis', 'statpacstatus', 
                'mdprobability', 'psdprobability', 'cpsdprobability', 'sfprobability', 
                'fovealthresholdprobability')
  
  odd_cols <- c('instrumentsoftwareversion', 'aedob', 'aeexamdate', 'eye', 'instrumentmodel', 
                'aeexamtime', 'trialrxsphereraw', 'trialrxcylraw', 'trialrxaxisraw', 
                'sfstatus', 'mdprob', 'psdprob', 'cpsdprob', 'sfprob', 
                'fovealthresholdprob')
...
if (length(l102) > 0) {
  ...
  if (all(c("BACKUP_TIME", "BACKUP_DATE") %in% names(o102))) {
  ...
  reg_cols <- c('hfa2softwareversion', 'birthdate', 'visitdate', 'site', 
                'examtime', 'statpacstatus')
  
  odd_cols <- c('instrumentsoftwareversion', 'aedob', 'aeexamdate', 'eye', 
                'aeexamtime', 'sfstatus')
...
kprs   <- which(c("o604", "o302", "o242", "o102") %in% ls())
checkr <- 0
for (i in kprs) {
  checkr <- checkr + nrow(get(c("o604", "o302", "o242", "o102")[i]))
}
